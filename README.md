# 潜力股雷达 · A股多因子共振选股工具

基于 **8 大信号源做多因子共振分析** 的 A 股潜力股筛选工具，支持任意个股的实时多维研判。

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/winccctan/potential-stock-radar)

> ⚠️ 本项目仅供学习与研究参考，**不构成任何投资建议**。股市有风险，入市需谨慎。

## 功能特性

### 1. 多因子共振选股（潜力股雷达）
用 8 类信号源做交叉验证，信号命中越多，趋势 + 资金 + 基本面同时看好的确定性越高：

| 信号源 | 说明 |
|---|---|
| 价值龙头 | 巴菲特式价值选股 |
| MACD 金叉 | 趋势反转信号 |
| 主力抢筹 | 大资金流入 |
| 业绩预增 | 业绩预告向好 |
| 高分评级 | 综合评分前列 |
| 资金 5 日流入 | 中期资金动向 |
| 高股息 | 分红回报 |
| 高盈利价值 | PB-ROE 匹配 |

- **信号热度雷达图**：可视化当前市场 8 类信号的覆盖强度
- **共振股榜单**：可按信号数 / 涨幅 / 市值 / PE 排序筛选
- **个股详情**：52 周价格区间、前瞻 PE、信号解读 + 风险提示

### 2. 个股速查（任意股票实时查询）
输入股票代码或名称，实时聚合多维度数据并自动生成研判：

- **代码/名称查询**：`600519`、`sh605006`、`贵州茅台` 均可，支持模糊联想
- **行情总览**：现价、PE/PB/股息率、换手/量比、5/10/20 日涨跌、52 周区间定位
- **技术信号 5 卡**：MACD、KDJ、RSI、BOLL、均线排列，每项带状态解读
- **资金动向 8 项**：今日/5日/10日/20日主力净流入 + 超大单/大单/中单/小单
- **筹码分布**：获利盘比例、平均成本、集中度
- **多因子三维研判**：量化面 + 资金面 + 位置面评分，自动生成风险提示

## 快速开始

```bash
# 1. 启动数据服务（端口 8890）
node server.js

# 2. 浏览器打开
open http://localhost:8890
```

页面内置 9 只共振股快照做兜底——即使服务未启动，也可直接打开 `潜力股雷达.html` 查询这 9 只股票。

### 在线版直连模式（无需本地服务）

在线静态托管（GitHub Pages）也能实时查询**任意 A 股**（沪 / 深 / 北交所）——页面内置前端直连引擎 `live-api.js`，浏览器直接请求腾讯 / 东方财富公开行情接口：

- **搜索**：腾讯智能搜索（代码 / 中文名 / 拼音）
- **实时行情**：腾讯行情（JSONP 直连）
- **日 K 线 / 技术指标**：腾讯 K 线接口 + 前端本地计算 MACD/KDJ/RSI/BOLL/MA
- **资金流向**：东方财富资金流日线（今日 / 5日 / 10日 / 20日主力净流入 + 大中小单）
- **筹码分布**：基于 K 线的估算值（页面标注"估算"）

页面启动时会自动探测：检测到本地 Node 服务（`/api/stock`）→ 使用完整后端数据；否则自动切换为直连模式，无需任何配置。

## 项目结构

```
├── 潜力股雷达.html     # 主页面（选股雷达 + 个股速查）
├── live-api.js         # 前端直连引擎（在线版全市场实时查询，零依赖）
├── server.js           # 数据服务（聚合行情/技术/资金/筹码数据）
├── vendor/             # 内置的数据查询引擎（自包含，无外部依赖）
├── index.html          # GitHub Pages 入口页
├── 山东玻纤股价监控.html # 个股监控看板示例
├── 山东玻纤趋势预测.html # 多因子趋势预测报告示例
├── package.json        # Node 服务配置（npm start）
└── render.yaml         # Render 云部署配置
```

## 在线访问

### GitHub Pages（在线直连版）
🔗 **https://winccctan.github.io/potential-stock-radar/**

- 在线版可直接打开查看三个页面（榜单数据为发布时快照）
- **任意股票实时查询**：自动启用直连模式（腾讯 / 东方财富公开接口），支持沪 / 深 / 北交所全市场 A 股
- 完整数据（含筹码分布实值、更丰富字段）建议本地运行 `node server.js` 或部署 Render

### Render（完整实时版）
部署后可通过公网链接访问，**任意股票实时查询** 全功能可用（与本地 `node server.js` 体验一致）。

## 部署到 Render（免费）

### ⚡ 一键部署（推荐）

点击上方 **Deploy to Render** 按钮（或直接访问 👉 [render.com/deploy?repo=winccctan/potential-stock-radar](https://render.com/deploy?repo=https://github.com/winccctan/potential-stock-radar)）：

1. 用 GitHub 账号登录 Render（首次需授权）
2. Render 自动读取仓库内 `render.yaml`，确认配置后点 **Apply**
3. 等待 2-3 分钟构建完成，即可访问 `https://potential-stock-radar.onrender.com`

无需填写任何表单——`render.yaml` 已预置 Build/Start 命令与免费套餐。

### 手动部署

1. 将本仓库推送到 GitHub（已完成）
2. 打开 [render.com](https://render.com) 并登录（可用 GitHub 账号）
3. **New** → **Web Service** → 连接 GitHub 并选择 `potential-stock-radar` 仓库
4. Render 会自动识别 Node 项目，关键配置：
   - **Build Command**: `npm install`
   - **Start Command**: `node server.js`
   - 地区选择 **Singapore**（靠近国内，延迟更低）
   - 免费实例每月 750 小时，15 分钟无访问会休眠，再次访问自动唤醒（约 30-60 秒）
5. 点击 **Create Web Service**，等待构建完成（约 2-3 分钟）
6. 完成后访问 Render 分配的 `https://xxx.onrender.com` 即可

> 也可用仓库内的 `render.yaml` 走 Blueprint 方式一键创建：New → Blueprint → 选择本仓库。

## 数据说明

- 行情数据基于 A 股公开市场数据，随交易日更新
- 技术指标（MACD/KDJ/RSI/BOLL/MA）与资金流向为接口实时计算
- 榜单数据为构建时快照，实时性以实际查询为准

## 技术栈

- 纯前端 HTML + 原生 JS（零依赖，可直接打开）
- Node.js 本地数据服务（仅依赖 Node 内置模块）
- 红涨绿跌（中国股市配色惯例）
